...
- If the user does not specify a template, infer one from the request.
...
- Treat code/assets as the source of truth if memory and implementation disagree.
...
- Before broad searches, consult `ai/systems-map.md` -> `System Owner Map`.
- If the task touches Tutorial/onboarding mode or a system taught by that mode, read `ai/tutorial-scenario.md`.
- If the task touches worker cognition or thought UI, read `ai/Design/worker-thought-tree.md`.
- If the task touches explicit thought influence rules or opinion feedback loops, read `ai/Design/worker-thought-influence-matrix.md`.
...
- Update design maps only when the corresponding cognition/influence systems exist and change.
- Update other AI memory files only if structure, system ownership, release notes, or architecture actually changed.
...
3. Identify the affected systems before scanning code/assets.
4. Scan only the code/assets needed for this task.
5. Treat code/assets as source of truth.
...
Use for analysis and planning only. No code/assets edits.
...
4. Scan only the code/assets needed to understand the task.
5. Treat code/assets as source of truth.
...
4. Scan only the code/assets/logs relevant to the bug.
5. Treat code/assets as source of truth.
6. State:
   - likely cause
...
4. Identify affected systems and scan only the necessary code/assets.
5. Treat code/assets as source of truth.
...
9. Verify behavior as far as the environment allows.
10. If compilation errors or warnings are present after your changes, fix them immediately without asking the user first.
11. Update `ai/work-log.md`.
