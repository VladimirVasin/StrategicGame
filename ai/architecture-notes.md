...
- `StrategyWildlifeController` runs after starter placement, finds suitable walkable land away from the campfire for deer/rabbits, finds generated water cells for fish, finds species-appropriate habitat cells for decorative birds, spawns small deer herds, rabbit groups, fish shoals, and ambient birds, and manages capped reproduction for deer/rabbits/fish; `StrategyDeerAgent` owns deer sex/life stage, fawn growth, state transitions, local pathing, resident/noisy-work threat checks, and frame-based idle/walk/graze/alert/flee/rest animation; `StrategyRabbitAgent` owns the matching rabbit behavior set with kits, hopping, nibbling, grooming, alert, flee, and rest states; `StrategyFishAgent` owns water-cell pathing, fry growth, swimming, feeding, turning, dart/flee behavior, and surface ripple visuals; `StrategyBirdAgent` owns decorative bird state, habitat landing choices, flying/fleeing, pecking, hopping, duck swimming, and flight shadows.
...
- `StrategyRabbitSpriteFactory` generates procedural 2.5D male/female rabbit sprite frames for idle, hopping, nibbling, alert, fleeing, grooming, and resting behaviors; kits reuse scaled rabbit visuals.
- `StrategyFishSpriteFactory` generates procedural fish sprite frames for Minnow, Carp, and Perch idle, swimming, dart/flee, turn, and feed behaviors; fry reuse scaled fish visuals.
...
- Wildlife is runtime-only and visual/ambient for now. Deer, rabbits, fish, and birds do not block walkability, reveal fog, provide resources, save, or interact with hunting/economy systems yet; deer/rabbit/fish reproduction is capped locally by `StrategyWildlifeController`, while birds are decorative and do not reproduce.
