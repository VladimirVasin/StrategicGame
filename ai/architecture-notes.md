...
- `StrategyConstructionSite` is the runtime bridge between placement and final buildings: it stores final tool metadata, target footprint, reserved/delivered resources, up to 2 builders, staged construction sprites, delivered material stockpiles, progress, and completion into a final placed building.
- `StrategyStorageYard` is the current storage/logistics worksite: it stores local Logs and Stone, supports up to 2 assigned workers, finds production camps with available camp stock, reserves Logs/Stone for haulers and construction sites, provides construction pickup cells, and updates visible Logs/Stone stockpiles.
...
- `StrategyBuildingUpgradeController` installs house upgrades near placed houses. It currently supports Garden Beds and Chicken Coop, assigns produced resources to upgrades, uses map walkability only as a placement filter, does not block cells after installation, and spawns idle chickens when Chicken Coop is installed.
- `StrategyBuildingUpgradeSpriteFactory` generates and caches runtime pixel-art sprites for the current visual upgrades.
- `StrategyPopulationController` creates a startup camp with an animated procedural campfire, spawns 3 male and 3 female initial residents, assigns random Germanic/Nordic-style full names, assigns temporary construction builders, and reserves one random free male/female pair for each house construction site while the free pool exists.
- `StrategyResidentAgent` stores a persistent runtime full name, drives simple idle movement around the current camp/home through short walkable grid paths using `CityMapController.IsCellWalkable`, and owns visual-only readability child renderers for a synced silhouette outline plus ground shadow; assigned residents also periodically path to their home Garden Beds, play a short work animation, and add the Garden Beds crop to the owning house when work completes. Residents assigned to a lumberjack camp path to nearby trees, play frame-based axe swings, send tree hit events on impact frames, wait for falling to finish, buck the fallen trunk into Logs, carry Logs to their camp stockpile, then plant saplings nearby. Residents assigned to construction sites fetch reserved Logs/Stone from Storage Yards, deliver materials, then build with frame-based hammer swings and timed progress hits.
...
- `StrategyResidentSpriteFactory` generates and caches 5 male and 5 female resident sprites, their walk frames, woodcut axe-swing frames, stonecut pickaxe frames, construction hammer/build frames, and matching portrait sprites at runtime.
...
- `StrategyWorldSelectionController` also shows storage yard worker slots, storage worker statuses, Logs/Stone stock, available source count, and construction-site cost/delivery/builder/progress context.
...
- House resources are runtime-only and local to each placed house. Garden Beds currently produce one assigned crop through resident work, and Chicken Coop passively produces Eggs; there is no global economy, save data, storage cap, trade, or consumption yet.
...
- Generated Stone deposits are runtime-only map resource nodes mined by assigned stonecutters; there is no Stone regrowth loop yet.
- Population is runtime-only. Residents currently idle locally around camp/home with short grid paths, move into houses from a finite starter-camp pool after house construction, can be manually assigned to work camps/storage yards, and can temporarily build construction sites, but do not have needs, save data, reproduction, immigration, or broader economy effects.
