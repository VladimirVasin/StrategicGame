...
- `StrategyWildlifeController` runs after starter placement, finds suitable walkable land away from the campfire for deer/rabbits, finds generated water cells for fish, finds species-appropriate habitat cells for decorative birds, spawns small deer herds, rabbit groups, fish shoals, and ambient birds, manages capped reproduction for deer/rabbits/fish, exposes adult-rabbit reservation/count lookup for hunter camps, and exposes catchable-fish reservation/count lookup for fisher huts; `StrategyDeerAgent` owns deer sex/life stage, fawn growth, state transitions, local pathing, resident/noisy-work threat checks, and frame-based idle/walk/graze/alert/flee/rest animation; `StrategyRabbitAgent` owns the matching rabbit behavior set plus hunter reservation, arrow-hit, death, carcass, and butchering-ready states; `StrategyFishAgent` owns water-cell pathing, fry growth, swimming, feeding, turning, dart/flee behavior, fishing reservation, hooked/caught states, and surface ripple visuals; `StrategyBirdAgent` owns decorative bird state, habitat landing choices, flying/fleeing, pecking, hopping, duck swimming, and flight shadows.
...
- `StrategyNatureSpriteFactory` generates and caches procedural 2.5D pixel-art sprites for large trees, small trees, bushes, forest groups, Stone boulders/clusters/cliffs, felled trunks, split Logs, carried Logs, carried `Дичь`, and carried `Рыба`.
...
- `StrategyHunterCamp` is the first hunting worksite: it supports up to 2 assigned hunters, reserves nearby adult rabbits through `StrategyWildlifeController`, receives delivered `Дичь`, and updates a local visual stockpile.
- `StrategyFisherHut` is the first fishing worksite: it supports up to 2 assigned fishers, reserves nearby adult fish through `StrategyWildlifeController`, chooses reachable shore cells for casting, receives delivered `Рыба`, and updates a local visual stockpile.
- `StrategyGranary` is the first food-storage/logistics worksite: it stores local `Дичь` and `Рыба`, supports up to 2 assigned food haulers, reserves available food from Hunter Camps/Fisher Huts, and updates separate visual food stockpiles.
...
- `StrategyBuildingSpriteFactory` now also generates Storage Yard Logs/Stone stockpile sprites, Hunter Camp `Дичь` stockpile sprites, Fisher Hut `Рыба` stockpile sprites, and Granary `Дичь`/`Рыба` stockpile sprites.
...
- `StrategyWorldSelectionController` handles left-click world selection for placed buildings/construction sites/residents, ignores the placement-completion frame, displays a runtime selection marker, owns the right-side selection HUD panel, shows selected-object previews/status/context, shows selected-house resident portrait rows, exposes compact house visual-upgrade action rows, shows selected-house resource icons/counts, and shows selected-lumberjack/stonecutter/hunter/fisher/granary worksite worker slots with assign/remove actions.
- `StrategyWorldSelectionController` also shows storage yard logistics-worker and builder slots, staff statuses, camp/storage stock, available source/target counts, and construction-site cost/delivery/builder/progress context.
...
- Granary `Дичь` and `Рыба` are runtime-only and local to each placed granary; food logistics centralizes production-building stock, but food is not consumed, spoiled, saved, traded, or exposed through a settlement-level stock service yet.
...
- Wildlife is runtime-only. Deer and birds are still ambient-only; rabbits can be reserved and hunted by hunter-camp workers for local `Дичь`, fish can be reserved and caught by fisher-hut workers for local `Рыба`, and wildlife still does not block walkability, reveal fog, save, or feed a global economy.
- Population is runtime-only. Residents currently idle locally around camp/home with short grid paths, move into houses from a finite starter-camp pool after house construction while that pool exists, can migrate from parental homes into empty houses as adult children, can form simple opposite-gender household pairs with kinship checks, can have children inside occupied houses, can be manually assigned to work camps/storage yards as adults, can hunt rabbits through Hunter Camps, fish through Fisher Huts, and can be hired as Storage Yard builders for construction, but do not have needs, save data, immigration, marriage UI, or broader economy effects.
