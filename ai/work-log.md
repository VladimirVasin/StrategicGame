...
- Added shared combat factions, targetability, damage/results, and deterministic integer health; residents now use a temporary Combat task with bow positioning/projectiles, while wolves support an exact forced target, timed bites, low-health retreat, and idempotent pack/list/migration cleanup.
- Added persistent 100-point resident combat health, wound-based movement penalties, one Dawn recovery per game day, save version 13 with v12 migration/validation, and fatal-damage routing through the existing assignment/death/corpse/funeral pipeline. Also completed missing Clay Pit, Kiln, Forge, and Forager Camp cleanup/profession snapshots on resident death.
- Added a reusable one-resident/one-wolf encounter controller that prefers an eligible Hunter, falls back to another adult, reports structured live-health/outcome state, and supports exact Reset/despawn cleanup.
- Added deterministic night encounter scheduling: one automatic wolf attack is eligible on every real `Dusk -> Night` transition from Day 2 onward, the first-night fauna story is protected, loads already inside Night do not replay an attack, modal/pause and active-battle gates defer safely, and failed staging retries without consuming the night. Dawn never ends a battle already in progress.
- Split F9 `Combat MVP` into localized RU/EN `Fight Now`, `Night Attack`, and `Reset` paths. `Fight Now` bypasses the calendar; `Night Attack` arms a guaranteed request, moves monotonically to displayed `21:50`, closes F9, and waits for the actual 22:00 Night transition and any first-night/modal sequence to finish.
- Blocked save capture throughout global `Active` and `Securing`, because live wolves, threat leases, corpses, and funeral progress remain transient in this slice; the localized HUD notice explains that saving resumes after battle peace. Encounter staging now creates the wolf before waking the selected defender, so an unavailable spawn cannot repeatedly interrupt Night rest.
- Changed the localization asset generator from clear/recreate to stable-ID upsert plus stale-key removal. Two consecutive official Unity generations produced byte-identical HUD assets, retained every one of the 625 pre-existing HUD IDs, and added only the 17 combat/save entries.
- Added the scene-global `Peaceful -> Active -> Securing -> Peaceful` lifecycle with idempotent threat leases and a 2.5-second scaled securing window. The F9 wolf holds its lease until actual despawn/removal, including retreat; temporary controller disable preserves the active battle, and direct controller replacement rebinds active leases.
- Resident death and corpse creation remain immediate during combat, while new funerals wait through `Active` and `Securing`. A battle suspends an existing funeral synchronously, releases mourners/carriers, torch and grave reservations, grounds the corpse in place, and restarts the process only after global peace.
- Verification: runtime and Editor builds pass with zero warnings/errors; focused lifecycle/funeral/night-scheduling/F9-wiring/combat/persistence/resident/localization EditMode coverage passes `83/83`, including `11/11` dedicated night tests; PlayMode bootstrap/refugee-routing smoke and technical quality gates pass. The earlier full EditMode run reached `479/515`; its one combat-adjacent activity-count expectation was fixed, while the other 35 failures are existing localization/UI/authored-import state failures outside this slice. Combat changes remain uncommitted for player review.
...
- Added a modal Yes/No arrival decision. Declining resolves the point immediately; accepting hands directly into a camera-focused five-second gameplay-space Scout rummaging cinematic with a dedicated 12-frame resident pose and exact visual cleanup.
- Added Holey Spoon as the first unique adult personal item, granted atomically to the exact Scout before a simulation-pausing personal reward card displays the generated spoon art and requested Russian copy. It appears through the existing selected-resident Personal Items HUD; no transfer, Use/Equip, effect, inheritance, or death-drop flow was added.
- Kept the story commitment and deferred expedition return active through the entire choice/cinematic/reward chain. Failed presentation entry stays unresolved and retryable; save v12 remains unchanged because the durable outcome is already represented by resolved story state plus the resident-owned item.
- Added focused content, asset-import, modal-handoff, five-second sequence, reward ownership, personal-item save, and story-resolution save coverage.
- Verification: technical quality gates pass and all five C# projects build sequentially with zero warnings/errors; the four focused encounter tests, existing dialog contract test, and PlayMode smoke test pass. The full EditMode suite is 415/431 with the same 16 unrelated baseline failures across snow, City Inventory layout, reward/audio presentation, fauna story cleanup, house smoke, input/UI feedback, characterization coverage, and audio import checks.
...
- Added `HouseholdPotteryReserveFailed` aggregate Storage Yard diagnostics with house demand, stored/available/reserved Pottery, yard count, dropoff availability, and failure reason.
- Added `HouseholderPotteryPickupCancelled` logs when an active household Pottery pickup is reset before completion, including carried amount, house/yard origins, store-carried flag, and cancellation reason.
...
- Changed household Pottery pickup so Storage Yard Haulers only bring Pottery into Storage Yard stock; Householders now reserve Pottery from Storage Yards and carry it into their own houses when cooking demand needs it.
- Kept raw `Fish`/`Game` on the same split: Haulers deliver food to Granaries, then Householders move house-bound food from Granaries to houses.
- Removed household Pottery delivery from Hauler logistics backlog while preserving raw household Pottery demand for Pottery production urgency.

### 2026-06-22 - Compact selected-house Dinner HUD

- Reworked the selected House food panel into a compact `Dinner` summary that shows current prepared rations vs family need, the next prepared/cookable dish, and the single most important blocker such as missing Pottery or missing food.
- Removed the old verbose Granary/ingredient reserve sentence from the top dinner block and replaced it with a short `Stock: Dishes | Pottery | Raw` line.
- Kept the detailed local house resource cards below the dinner block, but shortened their labels to amount/ration pairs so the panel stays readable.
- Verification: `dotnet build Assembly-CSharp.csproj -v:minimal` passed with 0 warnings and 0 errors; a runtime C# line-count scan found no files over 500 lines.

### 2026-06-20 - Household dish recipes

- Added a household dish recipe layer with 26 recipes across `Poor`, `Common`, `Hearty`, `Fine`, and `Feast` quality tiers.
- `StrategyHouseResourceStore` now stores prepared dishes as recipe stacks while preserving aggregate `Dish` amount/ration APIs for dinner, HUD, and logistics code.
- Householder cooking now chooses deterministic available recipes from house ingredients, consumes 1 Pottery per prepared dish, records recipe/quality/ration details in debug logs, and keeps direct ingredient fallback for dinners when dishes are missing.
- Selected House HUD now shows prepared dish ration totals and compact recipe summaries instead of a flat generic Dish count only.
- Verification: `dotnet build Assembly-CSharp.csproj -v:minimal` passed with 0 warnings and 0 errors; a runtime C# line-count scan found no files over 500 lines.

### 2026-06-20 - Pottery-required household dishes

- Changed household cooking so each prepared `Dish` now requires and consumes 1 `Pottery` in addition to ingredient rations.
- Nightly dinner now consumes prepared `Dish` first, then falls back to house-local ingredients when dishes are missing.
- Added household Pottery reservations from Storage Yard stock; this was later changed so Householders, not Haulers, perform the Storage Yard to House pickup.
- Updated house food HUD context, resident statuses, logistics backlog, and auto workforce Pottery urgency so missing household Pottery is visible to the simulation.
- Verification: `dotnet build Assembly-CSharp.csproj -v:minimal` passed with 0 warnings and 0 errors; a runtime C# line-count scan found no files over 500 lines.
...
- Split house food storage into ingredients and prepared dishes: Householders now cook ingredients into `Dish` during `Dusk`; later changes made dishes Pottery-gated and added house-local ingredient fallback for missing dishes.
