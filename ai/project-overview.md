...
- The top status HUD shows total population with separate adult and child counts; clicking it opens a larger residents roster with settlement stats, filters, age, home/camp state, role, current status, and food status. The roster can open a fullscreen paused Family Trees scene that groups recorded family members by connected kinship components, lays family groups out as affinity-ordered left-to-right columns with compact generation rows, and draws portrait-card parent-child trees with local parent-pair connectors, distinct deceased markers, always-visible gender symbols, and hover relationship labels. A compact top event log reports births, deaths, and adoptions.
