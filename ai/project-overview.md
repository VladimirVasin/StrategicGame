...
- A runtime nature-props layer places procedural 2.5D trees, forest groups, bushes, Stone deposits, and walkable mineable underground Iron/Coal fields over the generated terrain.
...
- The nature layer also places procedural Stone resource deposits as standalone boulders, rock clusters, and larger cliffs, plus mineable underground Iron and Coal fields as walkable multi-cell surface stains/seams that do not appear adjacent to the opposite mineral.
...
- Storage Yard is implemented as the first storage/logistics building: it has procedural 2.5D art, uncapped assigned storage workers/builders, uncapped local Logs, Stone, Iron, Coal, and Planks stock, growing visual stockpiles, resident hauling from production camps/Mines/Coal Pits/Sawmills, food hauling to Granaries, and construction resource reservations.
- Granary is implemented as the first food-storage/logistics building: it has procedural 2.5D art, up to 2 assigned workers, uncapped local `Game` and `Fish` stock, growing visual food stockpiles, and food hauling from Hunter Camps and Fisher Huts by Granary workers or Storage Yard workers.
...
- Player-built tools now construct through runtime construction sites: resources are reserved from Storage Yards, loose construction piles, and Stonecutter Camp Stone fallback; builders carry Logs/Stone/Planks to the site, residents animate hammer/build work, staged site sprites progress, and the final building appears only after progress completes.
...
- Completed Houses first try to move in one whole homeless family that fits, then fall back to one random free adult man and one random free adult woman from the camp instead of creating new residents; home assignment is independent from workplace or construction assignment.
...
- Residents assigned to a fisher hut walk to valid land/shore fishing cells, cast fishing lines toward reserved fish only while it remains in cast range, reel hooked fish in, carry `Fish`, and deposit it at the hut stockpile.
...
- Residents assigned to storage yards can reserve Iron from Mines, Coal from Coal Pits, and Planks from Sawmills, carry them to the Storage Yard, and deposit them into storage; the same storage workers can also haul `Game`/`Fish` to the nearest Granary.
...
- Generated Iron fields stay walkable because ore is underground, but Mines built over them can reserve and mine them into local Iron stock.
- Generated Coal fields stay walkable because coal is underground, but Coal Pits built over them can reserve and mine them into local Coal stock.
