...
- An optional `City Inventory` launcher in the top bar opens a read-only special-item chest with an English empty state and item details. The production catalog starts empty; opening the HUD blocks map/build input without pausing and does not gate or alter any founding, starter-build, or First Winter goal.
...
- Save version 8 retains the first-night fauna stage rather than individual runtime animals and adds deterministic City Inventory stacks; v7 saves migrate with an empty special-item chest. Loading an unresolved `MiceVisible` state reconstructs the mice and replays the rat prelude plus chronicle; legacy saves already at or beyond the first Night migrate as completed and do not replay the presentation.
