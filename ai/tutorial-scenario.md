...
- The board explains that Scouts travel continuously through day and night, reveal unknown territory, investigate landmarks, and report distant Iron/Coal sites. Resource landmarks remain simple fixed Coal/Iron discoveries. The separate ordered story sequence starts with a generated trash heap that materializes outside daylight visibility only after an exact Scout route succeeds. The introduction holds three stable random adult candidates, prioritizing current Haulers and Builders; an explicit appointment safely transfers the exact selected resident from an ordinary profession, while truly unavailable candidates remain visible with their blocking reason.
...
- Save version 12 persists the stable Lodge/resident assignment, Ready/Exploring/Returning state, absolute scaled start/end time, remaining field rations, fractional provision credit, daily nutrition guard, deferred story-return flag, story-anchor sequence/commitment state, and each adult's Personal Items stacks. Loading never pays again; an already expired Exploring save resumes as Returning unless its restored Scout must first finish a committed story point, while a restored Lodge never replays the first-expedition cinematic.
