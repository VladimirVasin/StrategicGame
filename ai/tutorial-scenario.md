...
- Selecting an adult resident shows a separate read-only `Personal Items` section with six slots and an empty state. Children do not show it; the initial production catalog is empty, so this surface has no grants, transfers, Use/Equip actions, or gameplay effects.
...
- The board explains that Scouts travel continuously through day and night, reveal unknown territory, investigate landmarks, and report distant Iron/Coal sites. Resource landmarks remain simple fixed Coal/Iron discoveries. Ordered story-landmark infrastructure is separate and currently has an empty production catalog, so the tutorial adds no example story discoveries. The introduction holds three stable random adult candidates, prioritizing current Haulers and Builders; an explicit appointment safely transfers the exact selected resident from an ordinary profession, while truly unavailable candidates remain visible with their blocking reason.
