...
- The board explains that Scouts travel continuously through day and night, reveal unknown territory, investigate landmarks, and report distant Iron/Coal sites. Resource landmarks remain simple fixed Coal/Iron discoveries. The separate ordered story sequence uses three camp-route-distance tiers; its first trash heap is Tier I, generated relatively near the camp so an early Scout can encounter it. Its hidden candidates do not block the map, and the heap materializes outside daylight visibility only after the Scout secures the exact route. The introduction holds three stable random adult candidates, prioritizing current Haulers and Builders; an explicit appointment safely transfers the exact selected resident from an ordinary profession, while truly unavailable candidates remain visible with their blocking reason.
- The same board chooses 1-7 full game days, defaults to one day, charges one ration per day before departure, and shows live cost, available eligible settlement/temporary/production food, projected return day/time, and a clear shortage warning. Household and loose-ground food never count toward this payment.
- If at least one adult is eligible and the selected duration is affordable, the introductory board requires a selection. If nobody can take the role or provisions are insufficient, `Decide Later` closes the board safely, selects the Lodge, and leaves its Assign action available for later.
- Appointment and first dispatch are one atomic action. The Scout enters `Exploring`, works continuously through day and night, consumes prepaid field rations instead of household dinner, and then enters `Returning` when the chosen duration expires or the player selects Recall. If the trash heap has already committed to that Scout, expiry or Recall waits for the complete encounter before return begins.
- Reaching the trash heap opens a paused Yes/No report. `Нет` closes the report and resolves the point without a reward. `Да, обыскать` focuses the camera on the Scout, runs about five seconds of rummaging sprite animation, reveals a Holey Spoon, adds it to that exact Scout's Personal Items, and opens a required reward card. Only closing the card resolves the point and releases deferred return.
- The selected Scout Lodge slot shows `Send`, `Recall`, or disabled `Returning`, along with live remaining time and field rations. Once the resident physically reaches the Lodge, the state becomes `Ready` and Send reopens the duration board without reassigning the resident; the Profession HUD summarizes Ready/Exploring/Returning counts and the resident HUD shows expedition progress.
- Scout appointment is manual-only and is not managed by Auto Workforce. It is not a starter-goal completion requirement: the storage stage begins as soon as Lodge construction completes.
- Save version 12 persists the stable Lodge/resident assignment, Ready/Exploring/Returning state, absolute scaled start/end time, remaining field rations, fractional provision credit, daily nutrition guard, deferred story-return flag, durable story-anchor sequence/commitment state, and each adult's Personal Items stacks. Logical hidden story candidates rebuild from the restored world and Fog state. Loading never pays again; an already expired Exploring save resumes as Returning unless its restored Scout must first finish a committed story point, while a restored Lodge never replays the first-expedition cinematic.
