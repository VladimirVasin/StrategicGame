...
- Save version 11 retains the first-night fauna stage and deterministic City Inventory stacks rather than individual runtime animals. The v8-to-v9 migration silently adds `Cats` to completed stories without replaying the reward; loading an unresolved `MiceVisible` state reconstructs the mice and replays the rat prelude, chronicle, and fresh reward flow.
...
- Appointment and first dispatch are one atomic action. The Scout enters `Exploring`, works continuously through day and night, consumes prepaid field rations instead of household dinner, and then enters `Returning` when the chosen duration expires or the player selects Recall.
...
- Save version 11 persists the stable Lodge/resident assignment, Ready/Exploring/Returning state, absolute scaled start/end time, remaining field rations, fractional provision credit, daily nutrition guard, and each adult's Personal Items stacks. Loading never pays again; an already expired Exploring save resumes as Returning, while a restored Lodge never replays the first-expedition cinematic.
