...
- Save version 10 retains the first-night fauna stage and deterministic City Inventory stacks rather than individual runtime animals. The v8-to-v9 migration silently adds `Cats` to completed stories without replaying the reward; loading an unresolved `MiceVisible` state reconstructs the mice and replays the rat prelude, chronicle, and fresh reward flow.
...
- Save version 10 persists the stable Lodge/resident assignment, Ready/Exploring/Returning state, absolute scaled start/end time, remaining field rations, fractional provision credit, and daily nutrition guard. Loading never pays again; an already expired Exploring save resumes as Returning, while a restored Lodge never replays the first-expedition cinematic.
