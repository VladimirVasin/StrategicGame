...
namespace ProjectUnknown.Strategy
{
    ...
    public sealed class StrategyFogOfWarController : MonoBehaviour
    {
        ...
        private void HandleToggleInput()
        {
            ...
            isPlayerFogEnabled = !isPlayerFogEnabled;
            if (fogRenderer != null)
            {
                fogRenderer.gameObject.SetActive(isPlayerFogEnabled);
            }
        ...
        private void CollectRevealSources()
        {
            ...
            {
                AddRevealSource(campCell, CampRevealRadius);
            }
            ...
            if (residents != null)
            {
                for (int i = 0; i < residents.Count; i++)
                {
                    ...
                    {
                        AddRevealSource(cell, ResidentRevealRadius);
                    }
                }
            }
            ...
            for (int i = 0; i < buildings.Count; i++)
            {
                ...
                revealSources.Add(new RevealSource(center, BuildingRevealRadius));
            }
        }
        ...
        private void ApplyRevealSources()
        {
            for (int i = 0; i < revealSources.Count; i++)
            {
                ...
                float outerRadius = source.Radius + RevealEdgeSoftness;
                int minX = Mathf.Max(0, Mathf.FloorToInt(source.CellCenter.x - outerRadius));
                int maxX = Mathf.Min(map.Width - 1, Mathf.CeilToInt(source.CellCenter.x + outerRadius));
                int minY = Mathf.Max(0, Mathf.FloorToInt(source.CellCenter.y - outerRadius));
                int maxY = Mathf.Min(map.Height - 1, Mathf.CeilToInt(source.CellCenter.y + outerRadius));
                ...
                for (int y = minY; y <= maxY; y++)
                {
                    for (int x = minX; x <= maxX; x++)
                    {
                        ...
                        float strength = 1f - Mathf.InverseLerp(source.Radius - 0.75f, outerRadius, distance);
                        ...
                        if (strength > visibilityStrength[x, y])
                        {
        ...
        private void AddRevealSource(Vector2Int cell, float radius)
        {
            ...
            revealSources.Add(new RevealSource(new Vector2(cell.x + 0.5f, cell.y + 0.5f), radius));
        }
        ...
        {
            public RevealSource(Vector2 cellCenter, float radius)
            {
