...
namespace ProjectUnknown.Strategy
{
    internal static partial class StrategyBuildingHudSnapshotFactory
    {
        private static void FillProcessorHeader(StrategyBuildingHudSnapshot snapshot, int workers, int maxWorkers, StrategyProfessionType profession, bool hasInput, int outputStored, StrategyBuildTool tool)
        {
            ...
            string state = workers <= 0 ? "Idle" : outputFull ? "Output full" : hasInput ? "Working" : "Needs input";
            ...
            snapshot.AddChip("state", "State", state, GetBuildingIcon(tool), tone);
            snapshot.AddChip("output", "Output", outputStored + "/" + StrategyProductionStorage.ProcessingOutputCapacity, GetBuildingIcon(tool), outputFull ? StrategyBuildingHudTone.Warning : StrategyBuildingHudTone.Info);
        }
        ...
        private static void SetProcessorStatus(StrategyBuildingHudSnapshot snapshot, int workers, bool hasInput, int outputStored)
        {
            ...
            if (workers <= 0)
            {
                snapshot.SetStatus("No worker assigned", "Assign this profession in the Professions HUD.", StrategyBuildingHudTone.Warning);
            }
            else if (outputFull)
            {
                snapshot.SetStatus("Output storage full", "Haulers need to collect finished goods.", StrategyBuildingHudTone.Warning);
            }
            else if (!hasInput)
            {
                snapshot.SetStatus("Waiting for inputs", "Required materials must be delivered before work continues.", StrategyBuildingHudTone.Warning);
            }
            else
            {
                snapshot.SetStatus("Production active", "Inputs are available and output storage has room.", StrategyBuildingHudTone.Positive);
            }
        }
        ...
        {
            string detail = incoming > 0 ? incoming + " incoming" : "Input material";
        ...
        {
            string detail = pending > 0 ? pending + " pending" : "Finished output";
        ...
        {
            snapshot.AddChip("workers", "Workers", workers + "/" + maxWorkers, StrategyProfessionIconFactory.GetIcon(profession), workers > 0 ? StrategyBuildingHudTone.Positive : StrategyBuildingHudTone.Warning);
        }
        ...
        private static void AddCapacityChip(StrategyBuildingHudSnapshot snapshot, StrategyResourceStore store)
        {
            ...
            snapshot.AddChip("capacity", "Stock", stored + "/" + capacity, null, stored >= capacity ? StrategyBuildingHudTone.Warning : StrategyBuildingHudTone.Info);
        }
        ...
        private static void AddResourceRow(StrategyBuildingHudSection section, StrategyResourceStore store, StrategyResourceType resource, int capacity = 0, string detail = "")
        {
            ...
            string reservation = reserved > 0 ? reserved + " reserved" : string.Empty;
            string rowDetail = string.IsNullOrEmpty(detail) ? reservation : string.IsNullOrEmpty(reservation) ? detail : detail + " · " + reservation;
        ...
        private static string GetWorkState(int workers, int sources, bool hasSpace) =>
            workers <= 0 ? "Idle" : !hasSpace ? "Stock full" : sources <= 0 ? "No sources" : "Working";
        ...
        private static string GetWorkStatusBody(int workers, int sources, bool hasSpace) =>
            workers <= 0 ? "Assign this profession in the Professions HUD." : !hasSpace ? "Haulers need to clear the local stock." : sources <= 0 ? "No usable sources are currently in the work area." : "Workers have sources available and local storage space.";
    }
}
